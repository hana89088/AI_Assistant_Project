...
class AIAssistant:
    ...
    async def _process_single_client(self, command, client):
        """Helper to process AI response and send for a single client concurrently"""
        try:
            response = await self.get_ai_response(command, client)
            await self.send_response(response, client)
        except Exception as e:
            logger.error(f"Error processing command for client: {e}")
    ...
    async def process_voice_command(self, command, websocket=None):
        ...
        if websocket:
            ...
            await self._process_single_client(command, websocket)
        else:
            await self.broadcast(status_msg)
            # If no specific client, we'll process it for all active clients
            # concurrently to avoid O(N) latency bottlenecks
            if self.clients:
                await asyncio.gather(
                    *[self._process_single_client(command, client) for client in self.clients],
