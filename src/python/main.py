...
class AIAssistant:
    ...
    async def broadcast(self, message):
        ...
        if self.clients:
            # ⚡ Bolt: Serialize JSON once instead of O(N) times to reduce CPU overhead
    ...
    async def process_voice_command(self, command, websocket=None):
        ...
        if websocket:
            ...
            # ⚡ Bolt: Process AI responses concurrently instead of sequentially
            # Reduces latency from O(N) to O(1) for shared microphone broadcasts
            ...
            if self.clients:
                await asyncio.gather(
                    *[process_client(client) for client in list(self.clients)],
                    return_exceptions=True
                )
