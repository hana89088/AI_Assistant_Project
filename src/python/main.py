...
class AIAssistant:
    ...
    async def broadcast(self, message):
        ...
        if self.clients:
            # ⚡ Bolt: Optimize WebSocket broadcasts
            # Serialize JSON once (O(1)) instead of per client (O(N))
            # Use return_exceptions=True so one failing client doesn't break the broadcast
