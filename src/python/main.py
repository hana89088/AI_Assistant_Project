...
class AIAssistant:
    ...
    async def broadcast(self, message):
        ...
        if self.clients:
            # Bolt: Extract json.dumps out of the loop to serialize only once
    ...
    async def process_voice_command(self, command, websocket=None):
        ...
        if websocket:
            ...
            # Bolt: Process clients concurrently instead of sequentially
            # Changes O(N) external API calls to O(1) latency
            ...
            if self.clients:
                await asyncio.gather(
                    *[process_client(c) for c in list(self.clients)],
